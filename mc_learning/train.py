...
class BPNeuralNetwork:
    ...
    def __init__(self, input_size, hidden_size, output_size, learning_rate=0.001, momentum=0.5):
        """
        初始化BP神经网络。
        Args:
    ...
    def train(self, train_loader, val_loader, epochs):
        ...
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(self.H, "trained_H_weights.pt")
                torch.save(self.U, "trained_U_weights.pt")
...
if __name__ == "__main__":
    ...
    # 加载数据集
    dataset = PathDataset(csv_file)

    # 假设 `dataset` 是一个包含所有数据的列表
    ...
    # 网络参数
    input_size = dataset.inputs[0].shape[0]  # 输入大小
    hidden_size = 1500
    output_size = dataset.outputs[0].shape[0]  # 输出大小

    # 初始化BP神经网络
    nn = BPNeuralNetwork(input_size, hidden_size, output_size, learning_rate=0.001, momentum=0.5)
